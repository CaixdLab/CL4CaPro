...
The gene mapping dictionary can be created using the *CPTAC3_mapping_gen.py* script. It may be necessary to manually verify certain mismatches between official symbol IDs and geneIDs due to updates in versions over recent decades. Once the mapping ID dictionary is finalized, the *CPTAC3_map.py* script can be utilized to produce the CPTAC-3 dataset. This dataset is then applied to validate models trained on TCGA data. Subsequently, the *GenerateFeatures_CPTAC3.py* script can generate CPTAC-3 features using TCGA-trained CL models to assess the performance of CPTAC-3.
